...
params = {
    ...
    'lr' : 0.00015,     # Learning rate for optimizers
    'beta1' : 0.7,      # Beta1 hyperparam for Adam optimizer
...
schedulerD = optim.lr_scheduler.ReduceLROnPlateau(optimizerD, mode='min', factor=0.2, patience=6, min_lr=min_lr, verbose=True)
schedulerG = optim.lr_scheduler.ReduceLROnPlateau(optimizerG, mode='min', factor=0.2, patience=6, min_lr=min_lr, verbose=True)
...
for epoch in range(params['nepochs']):
    for i, data in enumerate(dataloader, 0):
        ...
        schedulerD.step(errG.item() + errD.item())
        schedulerG.step(errG.item() + errD.item())
